...
background_img = pygame.image.load("assets/pictures/menuBg.png")  # Replace with your background image path

# Set initial window size and enable resizing
MIN_WIDTH, MIN_HEIGHT = 700, 600  # Minimum window size
WIDTH, HEIGHT = MIN_WIDTH, MIN_HEIGHT
screen = pygame.display.set_mode((WIDTH, HEIGHT), pygame.RESIZABLE)  # Ensure screen is initialized
pygame.display.set_caption("Farm Harvester")

# Colors
WHITE = (255, 255, 255)
SAND = (194, 178, 128)
HOUSE_COLOR = (150, 111, 51)
SELL_CELL_COLOR = (255, 215, 0)  # Gold color for sell cell
GREY = (200, 200, 200)
BLACK = (0, 0, 0)
...
game_running = False
cell_size = 100
grid_size = cell_size * 5  # Set a fixed grid size (5x5 cells)
...
# Load the alternate character image for animation
character_collect_img = pygame.image.load("assets/pictures/tiger_scythe_down.png")  # Replace with your alternate image path
...
# Crop growth timer and interval
growth_timer = 0  # Initialize timer
growth_interval = 5000  # Grow every 5000 milliseconds (5 seconds)
...
# Middle cell coordinates in the house grid (for selling corn)
sell_cell_position = (2, 2)

def initialize_crops():
    positions = set()

    while len(positions) < MAX_CROPS:  # Ensure we try to create up to MAX_CROPS
        pos = (random.randint(0, 4), random.randint(0, 4))
        positions.add(pos)  # Add to the set (duplicates won't be added)

    # Ensure each crop has a growth stage assigned (default to the first stage)
    return [{"position": pos, "stage": 0} for pos in positions]  # Create crops with unique positions
...
def draw_menu():
    ...
    # Draw title
    title_text = large_font.render("Farm Harvester", True, WHITE)
...
def draw_game():
    ...
    if current_grid == "main":
        ...
        screen.blit(to_town_text, (grid_x + (grid_size - to_town_text.get_width()) // 2, grid_y + 10))
        ...
        screen.blit(to_farm_text, (grid_x + (grid_size - to_farm_text.get_width()) // 2, grid_y + grid_size - cell_size + 10))
    ...
    money_text = default_font.render(f"$ {money_amount}", True, WHITE)
...
def main():
    ...
    while True:
        ...
        if game_running:
            ...
            elif pressed_keys["a"] and char_x > 0:  # Move left
                char_x -= char_speed
            ...
            if current_grid == "main":
                for crop in crops:
                    pos_x, pos_y = crop["position"]
                    crop_rect = pygame.Rect(grid_x + pos_x * cell_size, grid_y + pos_y * cell_size, cell_size, cell_size)
